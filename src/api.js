...
Api.prototype.setOpenid = function (openid, unbinded=1) {
    ...
    this.restoreUrl(openid);
}

Api.prototype.restoreUrl = function(openid = ''){
    ...
    const newParams={
        code: null,
        _authed: new Date().getTime()
    }
    if(openid){
        let newOpenid = this.store.get('openid')
        if(!newOpenid || newOpenid != openid){
            Toast.fail('Unsupport local storage!')
            newParams['openid']=openid
        }
    }
    locationurl = utils.replaceParam(locationurl, newParams);

    location.href = locationurl + '#' + backurl
}
