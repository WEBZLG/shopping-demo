<template>
  <div class="page order-page">
    <van-nav-bar title="积分订单" left-text="返回" left-arrow @click-left="onClickLeft" />
...
export default {
  ...
  methods: {
    ...
    viewOrder(order_id) {
      this.$router.push("/member/credit_order/detail?id=" + order_id);
    },
    loadOrder() {
      ...
      this.error = false;
      this.$api
        .post("member.credit_order/index", {
          status: status,
          page: this.page
        })
        .then(json => {
    ...
    cancelOrder(order_id) {
      this.$dialog
        ...
        .then(() => {
          this.$api
            .post("member.credit_order/cancel", { id: order_id })
            .then(json => {
              if (json.code == 1) {
    ...
    deleteOrder(order_id) {
      this.$dialog
        ...
        .then(() => {
          this.$api
            .post("member.credit_order/delete", { id: order_id })
            .then(json => {
              if (json.code == 1) {
                this.$toast.success(json.msg || "操作成功");
                this.changeStatus();
    ...
    payOrder(order_id) {
      this.$dialog
        ...
        .then(() => {
          this.$api.wxPay(order_id,'PO').then(()=>{
            this.changeStatus();
          }).catch(()=>{
    ...
    completeOrder(order_id) {
      this.$dialog
        ...
        .then(() => {
          this.$api
            .post("member.credit_order/confirm", { id: order_id })
            .then(json => {
              if (json.code == 1) {
                this.$toast.success(json.msg || "操作成功");
                this.changeStatus();
